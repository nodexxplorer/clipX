...
import {
  ...
  StyleSheet, ActivityIndicator, Keyboard, Dimensions, ScrollView,
} from 'react-native';
...
const TRENDING_TERMS = [
  'Action', 'Romance', 'Thriller', 'Sci-Fi', 'Comedy',
  'Horror', 'Animation', 'Drama', 'Anime', 'Documentary',
  'Fantasy', 'Crime',
];
...
export default function SearchScreen() {
  ...
  const [search, { data: searchData, loading: searching }] =
    useLazyQuery<{ searchMovies: { items: Movie[]; totalCount: number } }>(SEARCH_MOVIES);

  const { data: trendingData } =
    useQuery<{ trending: Movie[] }>(GET_TRENDING, { variables: { limit: 24 } });

  const { data: homeData } =
    useQuery<{ genres: Genre[] }>(GET_HOME_DATA, { variables: { trendingLimit: 1, popularLimit: 1 } });

  const genres: Genre[] = homeData?.genres ?? [];
  const isSearching = query.trim().length >= 2;

  // Auto-search if `q` param provided (deep-link from other screens)
  useEffect(() => {
    if (params.q?.trim()) {
      setQuery(params.q);
  ...
  // Tap a trending chip → run that search
  const handleTrendingTap = useCallback((term: string) => {
    setQuery(term);
    search({ variables: { query: term, limit: 40 } });
    Keyboard.dismiss();
  }, [search]);
  ...
  if (activeRating !== 'All') {
    ...
    results = results.filter(m => (m.voteAverage ?? 0) >= min);
  }
  if (activeYear !== 'All') {
    if (activeYear === '2010s') {
      results = results.filter(m => { const y = m.year ?? parseInt(m.releaseDate?.slice(0, 4) ?? '0'); return y >= 2010 && y <= 2019; });
    } else if (activeYear === '2000s') {
      results = results.filter(m => { const y = m.year ?? parseInt(m.releaseDate?.slice(0, 4) ?? '0'); return y >= 2000 && y <= 2009; });
    } else {
      results = results.filter(m => String(m.year ?? m.releaseDate?.slice(0, 4) ?? '') === activeYear);
    }
  }
  if (activeSort === 'rating')  results = [...results].sort((a, b) => (b.voteAverage ?? 0) - (a.voteAverage ?? 0));
  if (activeSort === 'newest')  results = [...results].sort((a, b) => (b.year ?? 0) - (a.year ?? 0));
  if (activeSort === 'popular') results = [...results].sort((a, b) => (b.popularity ?? 0) - (a.popularity ?? 0));
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      {/* ── Trending Searches (empty state only) ── */}
        ...
        <View style={styles.trendingSection}>
          ...
          >
            {TRENDING_TERMS.map(term => (
              ...
              >
                <Ionicons name="search-outline" size={12} color={colors.textMuted} style={{ marginRight: 4 }} />
        ...
        <View style={styles.filtersPanel}>
          ...
          <View style={styles.filterRow}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
              {SORT_OPTIONS.map(s => (
                <Pressable key={s.id} style={[styles.chip, activeSort === s.id && styles.chipActive]} onPress={() => setActiveSort(s.id)}>
                  <Text style={[styles.chipText, activeSort === s.id && styles.chipTextActive]}>{s.label}</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
          ...
          <View style={styles.filterRow}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
              <Pressable style={[styles.chip, !activeGenre && styles.chipActive]} onPress={() => setActiveGenre(null)}>
                <Text style={[styles.chipText, !activeGenre && styles.chipTextActive]}>All</Text>
              </Pressable>
              {genres.slice(0, 12).map(g => (
                <Pressable key={g.id} style={[styles.chip, activeGenre === g.id && styles.chipActive]} onPress={() => setActiveGenre(activeGenre === g.id ? null : g.id)}>
                  <Text style={[styles.chipText, activeGenre === g.id && styles.chipTextActive]}>{g.name}</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
          ...
          <View style={styles.filterRow}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
              {YEAR_OPTIONS.map(y => (
                <Pressable key={y} style={[styles.chip, activeYear === y && styles.chipActive]} onPress={() => setActiveYear(y)}>
          ...
          <View style={styles.filterRow}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
              {RATING_OPTIONS.map(r => (
                <Pressable key={r} style={[styles.chip, activeRating === r && styles.chipActive]} onPress={() => setActiveRating(r)}>
      ...
      {results.length > 0 ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySubText}>Try a different term or remove filters</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  filterDot:    { position: 'absolute', top: 0, right: 0, width: 7, height: 7, borderRadius: 3.5, backgroundColor: colors.primary },

  // ── Trending chips (new) ──
  trendingSection: { paddingHorizontal: spacing.xl, paddingTop: spacing.sm, paddingBottom: spacing.xs },
  trendingHeader:  { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.sm },
  trendingLabel:   { color: colors.textMuted, fontSize: fontSize.xs, fontWeight: fontWeight.bold, textTransform: 'uppercase', letterSpacing: 1 },
  trendingChips:   { gap: spacing.sm, paddingRight: spacing.xl },
  trendingChip:    {
  ...
  trendingChipText: { color: colors.textSecondary, fontSize: fontSize.sm },

  // ── Filters ──
  filtersPanel:   { backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border, paddingBottom: spacing.md },
  filterRow:      { paddingHorizontal: spacing.xl, paddingTop: spacing.md },
  filterLabel:    { color: colors.textMuted, fontSize: fontSize.xs, fontWeight: fontWeight.bold, textTransform: 'uppercase', letterSpacing: 1, marginBottom: spacing.sm },
  chips:          { gap: spacing.sm },
  chip:           { paddingHorizontal: spacing.md, paddingVertical: 6, backgroundColor: colors.surfaceLight, borderRadius: radius.round, borderWidth: 1, borderColor: colors.border },
  chipActive:     { backgroundColor: colors.primary, borderColor: colors.primary },
  chipText:       { color: colors.textSecondary, fontSize: fontSize.sm },
  chipTextActive: { color: '#fff', fontWeight: fontWeight.semibold },
  clearFilters:   { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginHorizontal: spacing.xl, marginTop: spacing.md },
  clearFiltersText:{ color: colors.error, fontSize: fontSize.sm },

  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: spacing.xl, marginTop: spacing.md, marginBottom: spacing.sm },
